...
The pipeline may process canonical KshetraAI tables and actual company-provided source files.

## Company-Provided Internal Source Files

These files are confidential source inputs and should be read from `private-data/`:
...
The pipeline may produce or validate:
...
# 6A. Source-to-Canonical Mapping

The company-provided files should be mapped into KshetraAI canonical views as follows:

| Canonical View/Table | Primary Source |
|---|---|
| representatives | `reps_territory.csv` |
| territories | `reps_territory.csv` |
| visit_entities | `retailers.csv`, `growers.csv` |
| crop_context | `growers.csv` |
| sales_signals | `retailer_pos.csv` |
| inventory_signals | `retailer_inventory_weekly.csv` |
| visit_history | `retailer_visit_log.csv` |
| campaign_engagement | `digital_funnel_weekly.csv`, `whatsapp_campaign.csv` |
| weather_signals | public source or controlled gap-fill |
| pest_signals | public source or controlled gap-fill |
| ndvi_signals | public source or controlled gap-fill |
| competitor_signals | public source, derived proxy, or controlled gap-fill |
| travel_signals | derived approximation or controlled gap-fill |
| recommendation_log | generated by KshetraAI |
| outcome_log | captured by KshetraAI workflow or controlled demo seed |

The data pipeline may create clean intermediate views.
It must not perform feature scoring, priority scoring, recommendation generation, anomaly detection, or explanation generation.
...
Allowed local confidential source path:

```text
private-data/
```

The pipeline may read from `private-data/`, but must not write derived outputs back into it.
...
Expected V1 internal clean views may include:
...
The AI may read `private-data/` when explicitly instructed, but must not modify, move, rename, or commit it.
...
- table structures
- ID consistency
- source-to-canonical mapping traceability
...
- mutate confidential source files
...
that prepares clean, validated company-provided internal data,
public agricultural inputs,
and controlled gap-fill sources
